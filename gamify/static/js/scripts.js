...
function generateData(dataset) {
  ...
  // pick the data 
  var a = dataset.map(function(d) {return d[document.getElementById('dd_left').value]});
  var b = dataset.map(function(d) {return d[document.getElementById('dd_right').value]});
 ...
 } else if (origin.slice(-1) == "live_monitoring") {	
 	a = a.slice(-60)
 	b = b.slice(-60)
 }
